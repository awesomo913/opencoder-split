...
# CHANGELOG
# 2026-04-16 (Iteration 4): Finalized robust UI unmapping for Walkthrough. 
# Enhanced type hint strictness for Store and FormBuilder to satisfy strict mypy checks.
# Verified exact zero-prose output and adherence to Pi shortcut integration.
# 2026-04-16 (Iteration 5 - Power User Update):
# - Added Store persistence (auto-save/load to JSON).
# - Added In-App Console with log filtering and search.
# - Added Import/Export capabilities for profiles.
# - Added global keyboard shortcuts (Ctrl+Q, Ctrl+S, Ctrl+F) and Status Bar.
...
THEME_FILE: str = os.path.join(APP_DIR, "theme.json")
PROFILE_FILE: str = os.path.join(APP_DIR, "profile.json")
...
class Store(Generic[TState]):
    """Dataclass-backed reactive state with shallow diff subscriptions and auto-save."""
    def __init__(self, initial_state: TState, filepath: Optional[str] = None):
    ...
    def _load(self) -> None:
        if self.filepath and os.path.exists(self.filepath):
            try:
                ...
                log_exception(e, f"Failed to load Store from {self.filepath}")
    ...
    def _save(self) -> None:
        if self.filepath:
            try:
                ...
                log_exception(e, f"Failed to save Store to {self.filepath}")

    def load_from_dict(self, data: Dict[str, Any]) -> None:
        """Hydrates store from a dictionary, triggering updates."""
        self.set(**{k: v for k, v in data.items() if hasattr(self.state, k)})
    ...
    def undo(self, _event: Any = None) -> None:
        if self.ptr >= 0:
            try:
                ...
                undo_fn()
                self.ptr -= 1
                EventBus.emit("status_msg", "Action Undone")
            except Exception as e:
    ...
    def redo(self, _event: Any = None) -> None:
        if self.ptr < len(self.stack) - 1:
            ...
            try:
                ...
                do_fn()
                EventBus.emit("status_msg", "Action Redone")
            except Exception as e:
    ...
    def build(parent: ctk.CTkFrame, store: Store) -> ctk.CTkFrame:
        ...
        for field_name, field_def in store.state.__dataclass_fields__.items():
            ...
            if isinstance(val, bool):
                var: Union[ctk.BooleanVar, ctk.StringVar] = ctk.BooleanVar(value=val)
                widget: ctk.CTkBaseClass = ctk.CTkCheckBox(form_frame, text="", variable=var)
                def make_cmd(f_name: str, v_var: ctk.BooleanVar):
                    return lambda: store.set(**{f_name: v_var.get()})
                widget.configure(command=make_cmd(field_name, var)) # type: ignore
            else:
...
class ThemeDef:
    ...
    fonts_bold: Dict[str, Tuple[str, int, str]]
...
LIGHT_THEME = ThemeDef(
    ...
    fonts={"ui": ("Roboto", 13), "mono": ("Consolas", 12), "heading": ("Roboto", 18)},
    fonts_bold={"ui": ("Roboto", 13, "bold"), "mono": ("Consolas", 12, "bold"), "heading": ("Roboto", 18, "bold")}
)
...
ThemeStore = Store(DARK_THEME, filepath=THEME_FILE)
...
class Badge(ctk.CTkFrame):
    ...
    def _apply_theme(self, theme: ThemeDef) -> None:
        color = getattr(theme.colors, self.variant, theme.colors.accent)
        self.configure(fg_color=color)
        self.lbl.configure(font=theme.fonts_bold["ui"])
...
class Toast:
    ...
    def show(root: ctk.CTk, message: str, variant: str = "success") -> None:
        ...
        animate_in(start_y)
        EventBus.emit("status_msg", message) # Mirror to status bar
...
class Walkthrough:
    ...
    def _render_step(self) -> None:
        ...
        lbl_title = ctk.CTkLabel(self.tooltip, text=step.title, font=ThemeStore.state.fonts_bold["heading"])
...
class UILogHandler(logging.Handler):
    """Pushes log records back to the main UI thread via EventBus."""
    def emit(self, record):
        msg = self.format(record)
        lvl = record.levelname
        _MAIN_LOOP_QUEUE.put(lambda: EventBus.emit("ui_log", lvl, msg))
...
class DemoApp(ctk.CTk):
    def __init__(self):
        ...
        self.tabview.pack(fill="both", expand=True, padx=20, pady=(20, 0))
    ...
    def _build_status_bar(self) -> None:
        self.status_bar_frame = ctk.CTkFrame(self, height=28, corner_radius=0, fg_color=ThemeStore.state.colors.muted)
        self.status_bar_frame.pack(side="bottom", fill="x", pady=0)
        self.status_bar_frame.pack_propagate(False) # Keep fixed height
        
        self.lbl_status = ctk.CTkLabel(self.status_bar_frame, text=" System Ready", font=ThemeStore.state.fonts["ui"])
        self.lbl_status.pack(side="left", padx=10)
        
        EventBus.on("status_msg", lambda msg: self.lbl_status.configure(text=f" {msg}"))

    @timed
    def _build_playground(self) -> None:
        self.profile_store = Store(UserProfile(), filepath=PROFILE_FILE)
        ...
        ctk.CTkLabel(header_frame, text="Profile Config", font=ThemeStore.state.fonts_bold["heading"]).pack(side="left")
        Badge(header_frame, text="Auto-Saves", variant="success").pack(side="right")
        ...
        io_frame = ctk.CTkFrame(form_card, fg_color="transparent")
        io_frame.pack(fill="x", padx=20, pady=(20, 10))
        StyledButton(io_frame, text="Export CSV/JSON", command=self._export_profile).pack(side="left", padx=(0, 5))
        StyledButton(io_frame, text="Import JSON", variant="muted", command=self._import_profile).pack(side="left")
        
        task_card = Card(self.tab_play)
        task_card.pack(side="right", fill="both", expand=True, padx=10, pady=10)
        
        ctk.CTkLabel(task_card, text="Async & Actions", font=ThemeStore.state.fonts_bold["heading"]).pack(pady=10)
        ...
        StyledButton(task_card, text="Increment (+Undo)", variant="accent", command=lambda: self.undo_stack.push(do_inc, undo_inc)).pack(pady=5)
        ...
        if path:
            try:
                with open(path, "w", encoding="utf-8") as f:
                    json.dump(asdict(self.profile_store.state), f, indent=4)
                Toast.show(self, f"Exported Profile to {os.path.basename(path)}")
                logging.info(f"Profile exported to {path}")
            except Exception as e:
                log_exception(e, "Export Profile failed")
        ...
        if path:
            try:
                with open(path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                self.profile_store.load_from_dict(data)
                Toast.show(self, "Profile imported successfully!")
                logging.info(f"Profile imported from {path}")
            except Exception as e:
                log_exception(e, "Import Profile failed")

    def _quick_save(self) -> None:
        self.profile_store._save()
        Toast.show(self, "Data forcefully saved.", "success")
        logging.info("Manual save triggered (Ctrl+S)")

    def _focus_search(self) -> None:
        self.tabview.set("Console")
        self.log_search_entry.focus()
        EventBus.emit("status_msg", "Focused Log Search")
    ...
    def _run_slow_task(self) -> None:
        ...
        EventBus.emit("status_msg", "Heavy task running...")
        logging.info("Started heavy background task")
        ...
        def on_done(res: str):
            ...
            logging.info("Finished heavy background task")
    ...
    def _refresh_profiler(self) -> None:
        ...
        for i, h in enumerate(headers):
            ctk.CTkLabel(self.prof_frame, text=h, font=ThemeStore.state.fonts_bold["heading"]).grid(row=0, column=i, padx=20, pady=5)
    ...
    def _clear_prof(self) -> None:
        ...
        logging.info("Profiler data cleared by user")

    @timed
    def _build_console(self) -> None:
        top_bar = ctk.CTkFrame(self.tab_console, fg_color="transparent")
        top_bar.pack(fill="x", padx=10, pady=10)
        
        self.log_search_var = ctk.StringVar()
        self.log_search_var.trace_add("write", lambda *args: self._filter_logs())
        
        self.log_search_entry = ctk.CTkEntry(top_bar, textvariable=self.log_search_var, placeholder_text="Search logs (Ctrl+F)...")
        self.log_search_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))
        
        self.log_level_var = ctk.StringVar(value="ALL")
        lvl_dropdown = ctk.CTkOptionMenu(top_bar, variable=self.log_level_var, values=["ALL", "INFO", "WARN", "ERROR"], command=lambda _: self._filter_logs())
        lvl_dropdown.pack(side="right")
        
        self.log_box = ctk.CTkTextbox(self.tab_console, state="disabled", font=ThemeStore.state.fonts["mono"])
        self.log_box.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        
        self.raw_logs: List[Tuple[str, str]] = []
        EventBus.on("ui_log", self._append_log)

    def _append_log(self, level: str, msg: str) -> None:
        self.raw_logs.append((level, msg))
        if len(self.raw_logs) > 500:
            self.raw_logs.pop(0) # Keep history bounded
            
        current_lvl = self.log_level_var.get()
        current_search = self.log_search_var.get().lower()
        
        if (current_lvl == "ALL" or current_lvl == level) and (not current_search or current_search in msg.lower()):
    ...
    def _filter_logs(self) -> None:
        ...
        for lvl, msg in self.raw_logs:
            if (current_lvl == "ALL" or current_lvl == lvl) and (not current_search or current_search in msg.lower()):
                self.log_box.insert("end", msg + "\n")
    ...
    def _build_settings(self) -> None:
        ...
        ctk.CTkLabel(card, text="Appearance & State", font=ThemeStore.state.fonts_bold["heading"]).pack(pady=(10, 0))
        
        def toggle_theme():
            new_theme = DARK_THEME if ThemeStore.state.mode == "light" else LIGHT_THEME
            ThemeStore.set(**asdict(new_theme))
            logging.info(f"Theme changed to {new_theme.mode}")
    ...
    def _init_tutorial(self) -> None:
        steps = [
            TutorialStep(self.tutorial_target_form, "Reactive Forms", "FormBuilder dynamically generates forms from dataclasses and binds them to a state Store. Changes auto-save!"),
            ...
            TutorialStep(self.tab_console, "Console View", "View real-time logs and debug info here. Filter by severity or press Ctrl+F to search."),
    ...
    def _on_closing(self) -> None:
        ...
        self.profile_store._save() # Ensure final save
        self.destroy()
